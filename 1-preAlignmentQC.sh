...
trap 'j=`ps aux | grep -P "$maindir" |grep -P "(fastqc)|(fastp)|(fastq_screen)|(reformat.sh)|(sortmerna)|(bgzip)|(bbmap)"| awk '"'"'{print $2}'"'"'`;kill -9 $j;kill -9 $(jobs -p);echo -e "\nKilling all background processes......\nExiting the script......\n";exit 1' SIGINT
...
do
  ...
  if [[ $layout == "SE" ]]; then
    ...
    if [[ -f $SortmeRNA_ref ]];then
      ...
      echo "+++++ $sample: SortMeRNA index not found !!! +++++"
    fi
