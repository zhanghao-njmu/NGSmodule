...
if (length(args) < 6) {
  stop("Usage: dupRadar.R <input.bam> <annotation.gtf> <strandDirection:0=unstranded/1=forward/2=reverse> <PE/SE> <nbThreads> <input_bam_basename>", call.=FALSE)
}
input_bam <- args[1]
annotation_gtf <- args[2]
stranded <- as.numeric(args[3])
paired_end <- if(args[4]=='PE') TRUE else FALSE
threads <- as.numeric(args[5])
outdir <- args[6]
input_bam_basename <- args[7]
...
# message("Output basename(Arg 6): ", input_bam_basename)


suppressWarnings(suppressPackageStartupMessages(invisible(lapply(c("dupRadar","parallel"), require, character.only = TRUE))))
...
sample_name <- input_bam_basename
line="#id: DupInt
...
write(line,file=paste0(input_bam_basename, "_dup_intercept_mqc.txt"),append = FALSE)
...
write(line,file=paste0(input_bam_basename, "_duprateExpDensCurve_mqc.txt"))
write.table(cbind(curve_x, curve_y),
