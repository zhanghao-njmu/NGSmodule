def RunSCVELO(adata=None, h5ad=None, group_by=None, liner_reduction=None, nonliner_reduction=None, dirpath="./", fileprefix="", dpi=300,
  ...
  try:
    ...
    if group_by is None or liner_reduction is None or nonliner_reduction is None:
      print("group_by, liner_reduction and nonliner_reduction must be all provided.")
      exit()
    if adata is None:
      adata = scv.read(h5ad)
    del adata.uns
    ...
    scv.pp.moments(adata, n_pcs=n_pcs, use_rep=liner_reduction,
    ...
    scv.pl.velocity_embedding_stream(adata, title="stochastic", basis=nonliner_reduction, vkey=[
                                     "stochastic"], color=group_by, smooth=stream_smooth, density=stream_density, save=False, show=True)
    plt.savefig(
        '.'.join(filter(None, [fileprefix, "stochastic_stream.png"])), dpi=dpi)
    scv.pl.velocity_embedding(adata, title="stochastic", basis=nonliner_reduction, vkey=[
                              "stochastic"], color=group_by, size=20, arrow_length=arrow_length, arrow_size=arrow_size, density=arrow_density, save=False, show=True)
    plt.savefig(
        '.'.join(filter(None, [fileprefix, "stochastic_arrow.png"])), dpi=dpi)

    scv.tl.velocity_confidence(adata, vkey="stochastic")
    scv.tl.velocity_pseudotime(adata, vkey="stochastic")
    scv.pl.scatter(adata, basis=nonliner_reduction, color=('stochastic_length', 'stochastic_confidence'),
                   cmap='coolwarm', perc=[5, 95], save=False, show=True)
    plt.savefig('.'.join(
        filter(None, [fileprefix, "stochastic_length_confidence.png"])), dpi=dpi)
    scv.pl.scatter(adata, basis=nonliner_reduction, color='stochastic_pseudotime',
                   cmap='gnuplot', save=False, show=True)
    plt.savefig(
        '.'.join(filter(None, [fileprefix, "stochastic_pseudotime.png"])), dpi=dpi)

    adata.uns['neighbors']['distances'] = adata.obsp['distances']
    adata.uns['neighbors']['connectivities'] = adata.obsp['connectivities']
    scv.tl.paga(adata, groups=group_by, vkey="stochastic")
    scv.pl.paga(adata, basis=nonliner_reduction[2:], threshold=paga_threshold, size=50, alpha=0.02,
                min_edge_width=2, node_size_scale=1.5, save=False, show=True)
    plt.savefig(
        '.'.join(filter(None, [fileprefix, "stochastic_paga.png"])), dpi=dpi)
    ...
    if calculate_velocity_genes is True:
      ...
      for cluster in df.columns:
        ...
        plt.savefig('.'.join(
            filter(None, [fileprefix, cluster, "stochastic_genes1.png"])), dpi=dpi)
        scv.pl.velocity(adata, color=group_by, var_names=df[cluster].values[:6], size=10, linewidth=2, alpha=1, ylabel="cluster: "+cluster+"\nunspliced",
                        add_outline=True, basis=nonliner_reduction, color_map=["Spectral", "YlOrRd"], ncols=2, save=False, show=False)
        plt.savefig('.'.join(
            filter(None, [fileprefix, cluster, "stochastic_genes2.png"])), dpi=dpi)

    if s_genes is not None and g2m_genes is not None:
      scv.tl.score_genes_cell_cycle(
          adata, s_genes=s_genes, g2m_genes=g2m_genes)
      scv.pl.scatter(adata, basis=nonliner_reduction, color=(
          'S_score', 'G2M_score'), smooth=True, perc=[5, 95], save=False, show=True)
      plt.savefig(
          '.'.join(filter(None, [fileprefix, "stochastic_cellcycle.png"])), dpi=dpi)
    ...
    if recover_dynamics is True or diff_kinetics is True or velocity_with_noise is True:
      ...
      scv.tl.velocity_graph(adata2, vkey="dynamical",
                            n_neighbors=n_neighbors, approx=approx)
      scv.pl.velocity_embedding_stream(adata2, title="dynamical", basis=nonliner_reduction, vkey=[
                                       "dynamical"], color=group_by, smooth=stream_smooth, density=stream_density, save=False, show=True)
      plt.savefig(
          '.'.join(filter(None, [fileprefix, "dynamical_stream.png"])), dpi=dpi)
      scv.pl.velocity_embedding(adata2, title="dynamical", basis=nonliner_reduction, vkey=[
                                "dynamical"], color=group_by, size=20, arrow_length=arrow_length, arrow_size=arrow_size, density=arrow_density, save=False, show=True)
      plt.savefig(
          '.'.join(filter(None, [fileprefix, "dynamical_arrow.png"])), dpi=dpi)

      scv.tl.velocity_confidence(adata2, vkey="dynamical")
      scv.tl.velocity_pseudotime(adata2, vkey="dynamical")
      scv.pl.scatter(adata2, basis=nonliner_reduction, color=('dynamical_length', 'dynamical_confidence'),
                     cmap='coolwarm', perc=[5, 95], save=False, show=True)
      plt.savefig('.'.join(
          filter(None, [fileprefix, "dynamical_length_confidence.png"])), dpi=dpi)
      scv.pl.scatter(adata2, basis=nonliner_reduction, color='dynamical_pseudotime',
                     cmap='gnuplot', save=False, show=True)
      plt.savefig(
          '.'.join(filter(None, [fileprefix, "dynamical_pseudotime.png"])), dpi=dpi)

      scv.tl.latent_time(adata2, vkey="dynamical")
      scv.pl.scatter(adata2, basis=nonliner_reduction, color='latent_time',
                     color_map='gnuplot', save=False, show=True)
      plt.savefig(
          '.'.join(filter(None, [fileprefix, "dynamical_latent_time.png"])), dpi=dpi)
      ...
      if calculate_dynamical_genes is True:
        ...
        for cluster in df.columns:
          ...
          scv.pl.scatter(adata, color=group_by, basis=df[cluster].values[:6], size=20, linewidth=2, alpha=1, ylabel="cluster: "+cluster+"\nunspliced",
                         add_linfit=True, add_rug=True, add_outline=True, ncols=3, frameon=True, save=False, show=False)
          plt.savefig(
              '.'.join(filter(None, [fileprefix, cluster, "dynamical_genes1.png"])), dpi=dpi)
          scv.pl.velocity(adata, color=group_by, var_names=df[cluster].values[:6], size=10, linewidth=2, alpha=1, ylabel="cluster: "+cluster+"\nunspliced",
                          add_outline=True, basis=nonliner_reduction, color_map=["Spectral", "YlOrRd"], ncols=2, save=False, show=False)
          plt.savefig(
              '.'.join(filter(None, [fileprefix, cluster, "dynamical_genes2.png"])), dpi=dpi)
      ...
      if diff_kinetics is True:
        ...
        scv.tl.velocity_graph(adata2, vkey="dynamical_kinetics",
                              n_neighbors=n_neighbors, approx=approx)
        scv.pl.velocity_embedding_stream(adata2, title="dynamical_kinetics", basis=nonliner_reduction, vkey=[
                                         "dynamical_kinetics"], color=group_by, smooth=stream_smooth, density=stream_density, save=False, show=True)
        plt.savefig(
            '.'.join(filter(None, [fileprefix, "dynamical_kinetics_stream.png"])), dpi=dpi)
        scv.pl.velocity_embedding(adata2, title="dynamical_kinetics", basis=nonliner_reduction, vkey=[
                                  "dynamical_kinetics"], color=group_by, size=20, arrow_length=arrow_length, arrow_size=arrow_size, density=arrow_density, save=False, show=True)
        plt.savefig(
            '.'.join(filter(None, [fileprefix, "dynamical_kinetics_arrow.png"])), dpi=dpi)
        ...
        scv.tl.velocity_graph(adata2, vkey="stochastic_kinetics",
                              n_neighbors=n_neighbors, approx=approx)
        scv.pl.velocity_embedding_stream(adata2, title="stochastic_kinetics", basis=nonliner_reduction, vkey=[
                                         "stochastic_kinetics"], color=group_by, smooth=stream_smooth, density=stream_density, save=False, show=True)
        plt.savefig('.'.join(
            filter(None, [fileprefix, "stochastic_kinetics_stream.png"])), dpi=dpi)
        scv.pl.velocity_embedding(adata2, title="stochastic_kinetics", basis=nonliner_reduction, vkey=[
                                  "stochastic_kinetics"], color=group_by, size=20, arrow_length=arrow_length, arrow_size=arrow_size, density=arrow_density, save=False, show=True)
        plt.savefig(
            '.'.join(filter(None, [fileprefix, "stochastic_kinetics_arrow.png"])), dpi=dpi)
      ...
      if velocity_with_noise is True:
        ...
        scv.tl.velocity_embedding(
            adata2, basis=nonliner_reduction[2:], vkey='dynamical_with_noise', autoscale=False)
        scv.pl.velocity_embedding_stream(adata2, title="dynamical_with_noise", basis=nonliner_reduction, vkey=[
                                         "dynamical_with_noise"], color=group_by, smooth=stream_smooth, density=stream_density, save=False, show=True)
        plt.savefig('.'.join(
            filter(None, [fileprefix, "dynamical_with_noise_stream.png"])), dpi=dpi)
        scv.pl.velocity_embedding(adata2, title="dynamical_with_noise", basis=nonliner_reduction, vkey=[
                                  "dynamical_with_noise"], color=group_by, size=20, arrow_length=arrow_length, arrow_size=arrow_size, density=arrow_density, save=False, show=True)
        plt.savefig('.'.join(
            filter(None, [fileprefix, "dynamical_with_noise_arrow.png"])), dpi=dpi)

      import os
      adata2.write(
          '.'.join(filter(None, [fileprefix, "dynamical.h5ad"])), compression='gzip')
...
def RunPAGA(adata=None, h5ad=None, group_by=None, liner_reduction=None, nonliner_reduction=None, dirpath="./", fileprefix="", dpi=300,
  ...
  try:
    ...
    sc.pp.neighbors(adata, n_pcs=n_pcs, use_rep=liner_reduction)
    ...
    if use_rna_velocity is True:
      ...
      sc.pl.paga_compare(adata, basis=nonliner_reduction, threshold=threshold, transitions="transitions_confidence",
      ...
      sc.pl.paga_compare(adata, basis=nonliner_reduction, threshold=threshold,
