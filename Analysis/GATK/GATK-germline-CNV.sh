...
echo -e "****************** Start GATK-germline-CNV ******************\n"
SECONDS=0
        ...
        *)
        ...
        while [[ $status == "uncompleted" ]] && (("$attempt" <= 0)); do
            ...
            if [[ $? == 1 ]]; then
                ...
                eval "$GATK3 -T HaplotypeCaller --emitRefConfidence GVCF -nct $threads -R $genome -D $dbSNP_snps -I ${dir_result}/BQSR/${prefix}.BQSR.bam -variant_index_type LINEAR -variant_index_parameter 128000 -o ${prefix}.gvcf.gz" &>>$dir_result/HaplotypeCaller/HaplotypeCaller.log
                eval "$GATK3 -T GenotypeGVCFs -nt $threads -R $genome --variant ${prefix}.gvcf.gz -o ${prefix}.vcf.gz" &>>$dir_result/HaplotypeCaller/HaplotypeCaller.log
            ...
            #     eval "$GATK3 -T VariantRecalibrator -nct $threads -R $genome -input $dir_result/HaplotypeCaller/${prefix}.HaplotypeCaller.vcf.gz $par_resource_snps -an QD -an MQ -an MQRankSum -an ReadPosRankSum -an FS -an SOR -an DP -mode SNP -recalFile ${prefix}.HaplotypeCaller.snps.recal -tranchesFile ${prefix}.HaplotypeCaller.snps.tranches -rscriptFile ${prefix}.HaplotypeCaller.snps.plots.R" &>>$dir_result/VQSR/VQSR.log
            #     eval "$GATK3 -T ApplyRecalibration -nct $threads -R $genome -input $dir_result/HaplotypeCaller/${prefix}.HaplotypeCaller.vcf.gz --ts_filter_level 99.0 -recalFile ${prefix}.HaplotypeCaller.snps.recal -tranchesFile ${prefix}.HaplotypeCaller.snps.tranches -mode SNP -o ${prefix}.HaplotypeCaller.snps.VQSR.vcf.gz" &>>$dir_result/VQSR/VQSR.log
            #     eval "$GATK3 -T VariantRecalibrator -nct $threads -R $genome -input ${prefix}.HaplotypeCaller.snps.VQSR.vcf.gz $par_resource_indels -an QD -an MQ -an MQRankSum -an ReadPosRankSum -an FS -an SOR -an DP -mode INDEL -recalFile ${prefix}.HaplotypeCaller.snps.indels.recal -tranchesFile ${prefix}.HaplotypeCaller.snps.indels.tranches -rscriptFile ${prefix}.HaplotypeCaller.snps.indels.plots.R" &>>$dir_result/VQSR/VQSR.log
            #     eval "$GATK3 -T ApplyRecalibration -nct $threads -R $genome -input ${prefix}.HaplotypeCaller.snps.VQSR.vcf.gz --ts_filter_level 99.0 -recalFile ${prefix}.HaplotypeCaller.snps.indels.recal -tranchesFile ${prefix}.HaplotypeCaller.snps.indels.tranches -mode INDEL -o ${prefix}.HaplotypeCaller.snps.indels.VQSR.vcf.gz" &>>$dir_result/VQSR/VQSR.log
            #     rm -f ${prefix}.HaplotypeCaller.snps.VQSR.vcf.gz
            #     mv ${prefix}.HaplotypeCaller.snps.indels.VQSR.vcf.gz ${prefix}.HaplotypeCaller.VQSR.vcf.gz
            ...
            # check_logfile "$sample" "FIlterVariantTranches" "$dir_result/FIlterVariantTranches/FIlterVariantTranches.log" "$error_pattern" "$complete_pattern" "precheck"
            # if [[ $? == 1 ]]; then
            #     rm -rf $dir_result/FIlterVariantTranches
            #     mkdir -p $dir_result/FIlterVariantTranches
            #     cd $dir_result/FIlterVariantTranches
            #     par_resource=$(printf -- " --resource '%s'" "${known_indels[@]}")
            #     eval "$GATK3 -T CNNScoreVariants -R $genome -V ${dir_result}/HaplotypeCaller/${prefix}.vcf.gz -O ${prefix}.score.vcf" &>>$dir_result/FIlterVariantTranches/FIlterVariantTranches.log
            #     eval "$GATK3 -T FilterVariantTranches -V ${prefix}.score.vcf " &>>$dir_result/FIlterVariantTranches/FIlterVariantTranches.log

            #     check_logfile "$sample" "FIlterVariantTranches" "$dir_result/FIlterVariantTranches/FIlterVariantTranches.log" "$error_pattern" "$complete_pattern" "postcheck"
            #     if [[ $? == 1 ]]; then
            #         continue
            #     fi
            # fi
            ...
            status="completed"
            color_echo "blue" "+++++ ${sample}: GATK-germline-CNV completed +++++"
        done
...
if [[ $ninterrupted != 0 ]]; then
    cat "$tmpfile" | grep "Interrupted" | uniq >$maindir/GATK-germline-CNV.Interrupted.txt
    color_echo "red" "\n\n################################################################################"
    color_echo "red" "    $ninterrupted of $total_task tasks interrupted."
    color_echo "red" "    Please check the samples in $maindir/GATK-germline-CNV.Interrupted.txt"
    color_echo "red" "################################################################################\n\n"
fi

ELAPSED="Elapsed: $(($SECONDS / 3600))hrs $((($SECONDS / 60) % 60))min $(($SECONDS % 60))sec"
echo -e "\n$ELAPSED"
echo -e "****************** GATK-germline-CNV Done ******************\n"
