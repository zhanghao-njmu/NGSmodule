...
[ $? -ne 0 ] && {
    color_echo "red" "Cannot find the command 'configureStrelkaGermlineWorkflow.py' from Strelka2.\n"
    exit 1
}

echo -e "############################# Strelka2 Parameters #############################\n"
echo -e "  Strelka2: $(which configureStrelkaGermlineWorkflow.py)\n"
...
echo -e "****************** Start Strelka2-germline-short-variant ******************\n"
SECONDS=0
...
for sample in "${arr[@]}"; do
    read -u1000
    {
        ...
        *)
        ...
        dir_result="$dir/Alignment-$Aligner/Strelka2-germline-short-variant"
        mkdir -p $dir_result
        cd $dir_result
        ...
        while [[ $status == "uncompleted" ]] && (("$attempt" <= 0)); do
            ...
            status="completed"
            color_echo "blue" "+++++ ${sample}: Strelka2-germline-short-variant completed +++++"
        done
...
if [[ $ninterrupted != 0 ]]; then
    cat "$tmpfile" | grep "Interrupted" | uniq >$maindir/Strelka2-germline-short-variant.Interrupted.txt
    color_echo "red" "\n\n################################################################################"
    color_echo "red" "    $ninterrupted of $total_task tasks interrupted."
    color_echo "red" "    Please check the samples in $maindir/Strelka2-germline-short-variant.Interrupted.txt"
    color_echo "red" "################################################################################\n\n"
fi

ELAPSED="Elapsed: $(($SECONDS / 3600))hrs $((($SECONDS / 60) % 60))min $(($SECONDS % 60))sec"
echo -e "\n$ELAPSED"
echo -e "****************** Strelka2-germline-short-variant Done ******************\n"
