suppressWarnings(suppressPackageStartupMessages(invisible(lapply(
  c("BiocParallel","edgeR","DESeq2","dplyr","stringr","scales","RColorBrewer",
    ...
    "gplots","openxlsx","ggalluvial","ggfittext","ComplexHeatmap","circlize","nord"), require, character.only = TRUE))))
...
count_bar <- function(lib_size,label){
  ...
  p <- ggplot(data = df,aes(x=x,y=y))+
    ...
    theme_pubr(border=T)+
    theme(aspect.ratio = 5/length(unique(df[,"x"])),
...
var_mean_plot <- function(count_matrix,group,label){
  ...
  df$group <- sapply(df$group, function(x){switch(as.character(x),
                                                  "Merge"="Merge",
  ...
  p <- ggplot(data = df,aes(x=log10(mean),y=log10(var)))+
    ...
    annotate(geom = "text",x = 0,y=Inf,hjust=0,vjust=3,
             label="Black: Poisson distribution",color="black")+
    annotate(geom = "text",x = 0,y=Inf,hjust=0,vjust=5,
...
MDplot<- function(count_matrix,group,up,down,nonsig,drop,label){
  ...
  df[,"group"] <- factor(df[,"group"],levels = c(paste0("Droped(",length(drop),")"),
                                                 paste0("Up-regulated(",length(up),")"),
                                                 paste0("Down-regulated(",length(down),")")))
  colors <- c("grey",theme_use[1],theme_use[2])
  names(colors) <- c(paste0("Droped(",length(drop),")"),
                     paste0("Up-regulated(",length(up),")"),
                     paste0("Down-regulated(",length(down),")"))
  ...
  p <- ggplot(data = df,aes(x=cpm,y=myClip(log2fc,min = -2,max = 2)))+
    ...
    geom_point(data = df2,aes(x=cpm,y=myClip(log2fc,min = -2,max = 2),fill=group),
...
filterNumRej_plot <- function(dds_res,max_padj,count_matrix,label){
  ...
  labels <- paste0("Quantile: ",gsub(x = names(filterThreshold),perl = T,replacement = "",pattern = "(?<=\\.\\d\\d).*(?=%)"),"\n",
  ...
  p <- ggplot(data = df,aes(x=x,y=y,color=group))+
    geom_segment(x=filterNumRej[names(filterThreshold),"x"],y=-Inf,
                 xend=filterNumRej[names(filterThreshold),"x"],yend=filterNumRej[names(filterThreshold),"y"],
                 linetype=3,inherit.aes = F)+
    geom_segment(x=-Inf,y=filterNumRej[names(filterThreshold),"y"],
                 xend=filterNumRej[names(filterThreshold),"x"],yend=filterNumRej[names(filterThreshold),"y"],
                 linetype=3,inherit.aes = F)+
    annotate("text",x=filterNumRej[names(filterThreshold),"x"]+0.01,y=filterNumRej[names(filterThreshold),"y"]/10,
...
volcano_plot <- function(count_matrix,res,col_padj,col_log2fc,col_DEgroup,DEgroup_level,max_padj,min_fc,label){
  cpm_matrix <- as.data.frame(cpm(count_matrix))
  cpm_cov <- apply(cpm_matrix, 1, function(x){sd(x)/mean(x)})
  df <- res
  rownames(df) <- res[,1]
  ...
  maxFC <-df %>% filter(.data[["log2fc"]] != Inf & .data[["log2fc"]] != (-Inf)) %>% 
    pull("log2fc") %>% abs() %>% max() %>% ceiling()
  ...
  p<- ggplot(data = df,aes(x=log2fc,y=-log10(padj),group=DEgroup))+
    ...
    scale_fill_manual(breaks=DEgroup_level,values = c(theme_use[1],theme_use[2],"grey80"),
                      labels=df_label)+
    scale_size_continuous(range = c(0.2,4))+
    labs(x="log2fc",y=paste0("-log10(padj)"),title = paste0("Volcano plot (",label,")"))+
    guides(fill=guide_legend(title = paste("DGEs:",sum(df_table[1:2])),override.aes = list(size = 3),order = 1),
...
heatmap <- function(LogNormCounts,genes,res,res_log2fc_col,res_padj_col,row_split,column_split,label,
  ...
  df_top_annotation <- HeatmapAnnotation(
    foo1 = anno_block(gp = gpar(fill = col_color),
  ...
  if (length(row_split)>0) {
    df_left_annotation <- HeatmapAnnotation(
      foo1 = anno_block(gp = gpar(fill = row_color),
  ...
  if (anno_zoom_box & !is.null(row_split) & !is.null(column_split)) {
    panel_fun = function(index, nm) {
      ...
      g = ggplot(d, aes(x = group, y = value,fill=group)) + 
        ...
        theme_pubr(border = T)+
        theme(aspect.ratio = 5/length(unique(column_split)),
    ...
    zoom_box <- HeatmapAnnotation(
      "anno_zoom_box" = anno_zoom(align_to = row_split, which = "row", panel_fun = panel_fun, 
  ...
  ht <- ComplexHeatmap::Heatmap(matrix = df,
  ...
  draw_ht <- function(){
    ...
    annotation_titles = c(protein_coding = "Protein-coding",
                          lincRNA = "lincRNA",
                          miRNA = "miRNA",
    ...
    decorate_annotation(annotation = "baseExp",{
      grid.lines(x = unit(c(0, 0),"native"), y = c(0, 1), gp = gpar(col = "black",lty = 2))
    },slice = 1)
    decorate_annotation(annotation = "log2fc",{
      grid.lines(x = unit(c(0, 0),"native"), y = c(0, 1), gp = gpar(col = "black",lty = 2))
    },slice = 1)
    if(length(unique(row_split))==2){
      decorate_annotation(annotation = "baseExp",{
  ...
  if (return_ggplot) {
    ...
    grob<- addGrob(gTree = grob,
                   child = textGrob(label = paste0("DGEs Heatmap\n(",label,")"),
                                    x = unit(0, "npc"), y = unit(0.98, "npc"),just="left"))
    p<- as_ggplot(grob)
    if (!anno_zoom_box) {
      add_width <- anno_genebiotype*5
      p <- p+theme(aspect.ratio = 40/(ncol(df)+36+add_width),
...
pie_chart <- function(res,annotation_matrix,label){
  ...
  df <- df%>% group_by(gene_biotype,group)%>%mutate(count=n())
  df <- df%>% group_by(group)%>%mutate(perc=count/n())
...
gene_test_plot <- function(test,drop_low,drop_outlier=NULL,annotation_matrix,label){
  df <- data.frame(gene=c(test,drop_low,drop_outlier),
                   class=c(rep("Tested",length(test)),
                           rep("Droped.LowExp",length(drop_low)),
                           rep("Droped.Outlier",length(drop_outlier))),stringsAsFactors = F)
  ...
  df <- df%>% group_by(gene_biotype,class)%>%mutate(count=n())
  df <- df%>% group_by(class)%>%mutate(perc=count/n(),total_count=n())
  ...
  p <- ggplot(df,aes(x=class,y=count,fill=gene_biotype))+
    ...
    geom_text(aes(label=ifelse(perc>0.15,paste0(round(perc*100,1),"%\n","(",count,")"),"")),
              color="white",size=3.5,stat="identity",position = position_stack(vjust=0.5))+
    geom_text(data = unique(df[,c("class","total_count")]),
    ...
    guides(fill=guide_legend(ncol=4, byrow=F))+
    theme_pubr(border = T)+
    theme(aspect.ratio = 0.5,plot.margin = margin(r = 100),
          legend.key.width=unit(4,"mm"),
          legend.key.height=unit(4,"mm"),
          strip.text = element_text(size=12),
          panel.grid.major = element_line(),
...
scale_factor_compare <- function(edgeR_sf,DESeq2_sf,lib_size){
  ...
  p1 <- ggplot(data = df1,aes(x=Sample,y=value))+
    ...
    theme_pubr(border=T)+
    theme(aspect.ratio = 6/length(unique(df1[,"Sample"])),
...
cpm_gene_curve<- function(count_matrix,cpm_seq,label,minCPM,minCPM_label){
  cpm_matrix <- as.data.frame(cpm(count_matrix))
  df<- apply(cpm_matrix,2,function(y){sapply(cpm_seq,FUN = function(x){sum(y>x)})})
  df <- as.data.frame(df)
  df[,"CPM"] <- cpm_seq
  ...
  p <- ggplot(data = df,aes(x=CPM,y=value,color=variable))+
    ...
    geom_vline(data = data.frame(minCPM=minCPM,minCPM_label=minCPM_label),
...
alluvial_plot <- function(res_list,annotation_matrix,tool,stratum_col){
  ...
  compare_df <- compare_df%>%group_by(RowID,DifferentialExpression)%>% mutate(count=n())
  ...
  p <- ggplot(compare_df,
              aes(x = name,y = unit,stratum = compare_df[,stratum_col], alluvium = RowID,
    ...
    theme_pubr()+
    theme(aspect.ratio = 2/length(compare),
          axis.line = element_blank(),
          axis.title = element_blank(),
          axis.ticks = element_blank(),
