...
class StatsService:
    ...
    def get_project_stats(self) -> ProjectStats:
        ...
        active = self.db.query(func.count(Project.id)).filter(
            Project.status.in_(['active', 'running'])
        ).scalar() or 0
        ...
        failed = self.db.query(func.count(Project.id)).filter(
    ...
    def get_sample_stats(self) -> SampleStats:
        ...
        processing = self.db.query(func.count(Sample.id)).filter(
            Sample.status.in_(['processing', 'running'])
        ).scalar() or 0

        completed = self.db.query(func.count(Sample.id)).filter(
            Sample.status == 'completed'
        ).scalar() or 0

        failed = self.db.query(func.count(Sample.id)).filter(
    ...
    def get_task_stats(self) -> TaskStats:
        ...
        running = self.db.query(func.count(Task.id)).filter(
            Task.status == 'running'
        ).scalar() or 0
        ...
        cancelled = self.db.query(func.count(Task.id)).filter(
    ...
    def get_storage_stats(self) -> StorageStats:
        ...
        projects_space = self.db.query(func.sum(File.size)).join(
            Sample, File.sample_id == Sample.id
        ).join(
            Project, Sample.project_id == Project.id
        ).scalar() or 0
    ...
    def get_user_activity_stats(self) -> UserActivityStats:
        ...
        active_month = self.db.query(func.count(User.id)).filter(
    ...
    def get_pipeline_stats(self) -> PipelineStats:
        ...
        completed_runs = self.db.query(func.count(Task.id)).filter(
            Task.status == 'completed'
        ).scalar() or 0
        ...
        most_used_list = [
            {"pipeline_id": str(pipeline_id), "count": count}
            for pipeline_id, count in most_used
        ]
    ...
    def get_quick_stats(self, user_id: Optional[str] = None) -> QuickStats:
        ...
        total_samples = self.db.query(func.count(Sample.id)).join(
            Project, Sample.project_id == Project.id
        ).filter(*query_filter).scalar() or 0

        running_tasks = self.db.query(func.count(Task.id)).filter(
            Task.status == 'running'
        ).scalar() or 0
    ...
    def get_trend_data(
        ...
        for i in range(days):
            ...
            if metric == "projects":
                ...
                count = self.db.query(func.count(Sample.id)).filter(
                    Sample.created_at >= day,
                    Sample.created_at < next_day
                ).scalar() or 0
            elif metric == "tasks":
