...
from app.schemas.user import SystemStats, UserAdminUpdate, UserResponse, UserStats, UserUpdate
...
async def list_users(
    skip: int = 0, limit: int = 100, current_admin: User = Depends(get_current_admin), db: Session = Depends(get_db)
):
    """
...
async def get_user(user_id: str, current_admin: User = Depends(get_current_admin), db: Session = Depends(get_db)):
    """
...
async def toggle_user_active_status(
    user_id: str, current_admin: User = Depends(get_current_admin), db: Session = Depends(get_db)
):
    """
...
async def get_user_stats(user_id: str, current_admin: User = Depends(get_current_admin), db: Session = Depends(get_db)):
    """
    ...
    total_samples = db.query(func.count(Sample.id)).join(Project).filter(Project.user_id == user_id).scalar() or 0
...
async def get_system_stats(current_admin: User = Depends(get_current_admin), db: Session = Depends(get_db)):
    """
    ...
    running_tasks = db.query(func.count(PipelineTask.id)).filter(PipelineTask.status == "running").scalar() or 0

    completed_tasks = db.query(func.count(PipelineTask.id)).filter(PipelineTask.status == "completed").scalar() or 0

    failed_tasks = db.query(func.count(PipelineTask.id)).filter(PipelineTask.status == "failed").scalar() or 0

    # Sum storage
    storage_stats = db.query(
        func.sum(User.storage_used).label("total_used"), func.sum(User.storage_quota).label("total_quota")
    ).first()
