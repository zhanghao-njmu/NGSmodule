...
async def create_samples_batch(
    ...
    count = service.create_batch(
        ...
        samples_data=batch_data.samples
    )

    return MessageResponse(
        message=f"Successfully created {count} samples",
        detail=f"Added to project"
    )
...
async def import_samples_from_csv(
    ...
    count, project_name = service.import_from_csv(
        user_id=current_user.id,
        project_id=project_id,
        csv_content=content
    )
    ...
    return MessageResponse(
        message=f"Successfully imported {count} samples",
        detail=f"Added to project '{project_name}'"
    )
